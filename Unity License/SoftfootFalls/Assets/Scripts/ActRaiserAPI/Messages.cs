...
public abstract class Messages : MonoBehaviour
{
    ...
    // 1c. miracle *** yes no
    // 1d. miracle *** show me where (not Wind or Earthquake)
    // 2. Menus.
    // 2a. Start Menu
    ...
    private static readonly string[] quest4 = new string[]
    {
        new StringBuilder()
    ...
    private static string[] quest2 = new string[] 
    {
        new StringBuilder()
            .Append("Sir ")
            .Append(PlayerName)
    ...
    public static string[] Quest4 { get { return quest4; } }
    ...
    private static string playerName = "________";
    ...
    public static bool IncrementIndex(int length)
    {
        ...
        if (Index >= length)
        {
            Timers.scrollIndex = 0;
            Timers.scrollTime = 0.0f;
            Index = 0;
            result = true;
        }
        return result;
