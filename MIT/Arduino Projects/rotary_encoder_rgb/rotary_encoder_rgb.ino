...
// https://github.com/sparkfun/Rotary_Encoder_Breakout-Illuminated/blob/main/README.md
...
#define ROT_LEDG A1     // green LED // TODO: Remember Hardware broken.
...
#define LED_R 9 // TODO: Remember Hardware broken.
#define LED_G A5
#define LED_B 6
...
const String OUTGOING_START = "type:config,filename:rotary_encoder_rgb,function:reset,delimiters: cm0; cm1; cm2; cm3; cm4;\n";
int incomingMessage;
...
void setup()
{
  ...
  Serial.begin(BAUD_RATE); // Use serial for debugging
  ...
  //PCintPort::attachInterrupt(ROT_SW, &buttonIRQ, CHANGE); // TODO: Remember that this is super broken for some reason.
  //attachInterrupt(ROT_SW, buttonIRQ, CHANGE); // TODO: Remember that this doesn't work, as is.
}
...
// Set RGB LED to one of eight colors (see #defines above)
{
  ...
  digitalWrite(LED_R, color & B001);
  digitalWrite(ROT_LEDG, color & B010);
  digitalWrite(LED_G, color & B010);
  digitalWrite(ROT_LEDB, color & B100);
  digitalWrite(LED_B, color & B100);
}
